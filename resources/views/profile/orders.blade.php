        ...
        <section>
            ...
            <div class="w-full space-y-4 text-white text-sm text-left">
                @foreach($orders as $order)
                    <?php /** @var \Src\Domain\Order\Models\Order $order */ ?>
                    <!-- Order item -->
                        ...
                        <div class="py-4">
                            <div class="flex gap-6">
                                <div class="shrink-0 overflow-hidden w-[64px] sm:w-[84px] h-[64px] sm:h-[84px] rounded-2xl">
                                    <img src="{{ $order->orderItems->first()->product->thumbnail }}" class="object-cover w-full h-full" alt="Заказ №{{ $order->id }}">
                                </div>
                                <div class="grow py-2">
                                    <div class="flex flex-col md:flex-row md:items-center gap-2">
                                        <h4 class="pr-3 text-md font-bold"><a href="orders-item.html" class="inline-block text-white hover:text-pink">Заказ №{{ $order->id }}</a></h4>
                                        <div class="px-3 py-1 rounded-md bg-{{ $order->status->color() }} text-xxs">{{ $order->status->humanValue() }}</div>
                                        <div class="px-3 py-1 rounded-md bg-white/10 text-xxs">{{ $order->created_at->format('d.m.Y') }}</div>
                                    </div>
                                    <div class="mt-3 text-body text-xs">На сумму: {{ $order->total->getFormattedValue() }} ₽</div>
                                </div>
                            </div>
                        </div>
                        <div class="py-4">
